...
public class CalendarUIListing : MonoBehaviour
{
    ...
    public void RenderCalendar(GameTimestamp timestamp)
    {
        ...
        for(; entryIndex < lastDay; entryIndex++)
        {
            ...
            if (seasonsTime.day == today.day &&
              //seasonsTime.year == timestamp.year &&
              seasonsTime.season == today.season &&
              seasonsTime.GetDayOfTheWeek().ToString() == today.GetDayOfTheWeek().ToString())
            {
                isToday = true;
            }
            ...
            {
                entries[entryIndex].Display(seasonsTime.day, seasonsTime.GetDayOfTheWeek(), charactersBirthday.portrait, charactersBirthday.name +"'s birthday", isToday);
            } else
            {
                entries[entryIndex].Display(seasonsTime.day, seasonsTime.GetDayOfTheWeek(), null, "",isToday);
            }
